...
@interface ZhanqiViewController ()<UICollectionViewDelegate,UICollectionViewDataSource>{
    recommendCollectionModel *_recommendCollection;
}
@property (nonatomic, strong) UICollectionView *collectionView;
@end

@implementation ZhanqiViewController
    ...
    if (indexPath.section == 1) {
        if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
            ...
            headerView.goMoreSeedingBlock = ^{
                ...
                listVC.title = @"战旗LOL直播间";
                listVC.type = TVTypezhanqi;
                listVC.hidesBottomBarWhenPushed = TRUE;
                [self.navigationController pushViewController:listVC animated:TRUE];
            };
            headerView.title = @"刘脑残看直播啦";
            return headerView;
